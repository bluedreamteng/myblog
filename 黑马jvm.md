---
title: 黑马解密JVM
date: 2023-02-22 15:20:05
tags: JVM
---

 黑马解密JVM相关笔记

 视频地址：[解密JVM](https://www.bilibili.com/video/BV1yE411Z7AP/)
...
![](黑马jvm/微信截图_20230223114256.png)

## 4 学习路线

![](黑马jvm/微信截图_20230223114935.png)
...
![](黑马jvm/微信截图_20230223121429.png)

- 不会存在内存溢出

## 2 虚拟机栈

![](黑马jvm/微信截图_20230223121804.png)
...
```
cn.itcast.jvm.t1.stack.Demo1_2
...
由控制台输出可以看出，在**Xss**数值为**256k**时，经历**3749**次方法调用后出现**StackOverflowError**

现在修改**Xss**数值为**128k**

![](黑马jvm/微信截图_20230223163013.png)
...
由控制台输出可以看出，在**Xss**数值为**128k**时，经历**1087**次方法调用后出现**StackOverflowError**
...
- Linux环境下运行某些程序的时候，可能导致CPU的占用过高，这时需要定位占用CPU过高的线程
  ...
  - **ps H -eo pid, tid（线程id）, %cpu | grep 刚才通过top查到的进程号** 通过ps命令进一		步查看是哪个线程占用CPU过高
  - **jstack 进程id** 通过查看进程中的线程的nid，刚才通过ps命令看到的tid来**对比定位	**，注意jstack查找出的线程id是**16进制的**，**需要转换**

## 3 本地方法栈

![](黑马jvm/微信截图_20230223165159.png)
...
java.lang.OutOfMemoryError: Java heap space
	at java.util.Arrays.Of(Arrays.java:3332)
...
![](黑马jvm/微信截图_20230224231814.png)

由图可以看出

第一阶段初始占用量

第二阶段由于引用了一个10mb的数组导致堆内存使用量上升

第三阶段由于垃圾回收器的作用，导致堆内存使用量下降
...
![](黑马jvm/微信截图_20230224233448.png)

当多次点击执行垃圾回收按钮时，发现回收内存总是有限，这时我们点击 **堆Dump**按钮进一步分析问题

![image-20230224235139321](黑马jvm/image-20230224235139321-1677253906851-2.png)

然后点击上图查找按钮

![image-20230224235231391](黑马jvm/image-20230224235231391-1677253953687-4.png)

可以看出占用空间最大的是一个ArrayList对象

![image-20230224235328796](黑马jvm/image-20230224235328796.png)

进一步查看发现，是由于ArrayList对象里面的Student对象里的big字段过大导致的。

这就是通过JVisualVM定位堆内存过大的基本排查方法。

## 5 方法区

![image-20230225001428818](黑马jvm/image-20230225001428818-1677255272972-6.png)

### 5.1 定义

[JVM规范-方法区定义](https://docs.oracle.com/javase/specs/jvms/se8/html/jvms-2.html#jvms-2.5.4)

![image-20230225001910688](黑马jvm/image-20230225001910688.png)


### 5.2 结构

![image-20230225002044215](黑马jvm/image-20230225002044215.png)
...
![image-20230225115043146](黑马jvm/image-20230225115043146.png)
...
框架中广泛使用的字节码技术，动态类加载可能会导致永久代内存溢出，元空间可能性较小，因为其使用的是系统内存。
...
将此类编译为class，找到class的绝对路径路径，应用如下命令

```
javap -v D:\worksapce\hm_jvm\jvm\target\classes\cn\itcast\jvm\Test.class
```

即可得到反编译以后类的信息

![image-20230225122757059](黑马jvm/image-20230225122757059.png)
...
- 运行时常量池
  常量池是*.class文件中的，当该**类被加载以后**，它的常量池信息就会**放入运行时常量池**，并把里面的**符号地址变为真实地址**，符号地址就是如 #1 #2 这一类的地址，放入运行时常量池后要把这些地址转换为内存的真实地址。
...
String ab = "ab";
String ab2 = a+b;
//结果为false,因为ab是存在于串池之中，ab2是由StringBuffer的toString方法所返回的一个对象，存在于堆内存之中，二者返回的是不同的对象
...
![image-20230227170127751](黑马jvm/image-20230227170127751.png)
...
![永久代空间溢出](黑马jvm/image-20230227171322109.png)

jdk1.8

![堆空间溢出](黑马jvm/image-20230227171412509.png)

#### 5.6.5 StringTable 垃圾回收
...
```
//触发了垃圾回收操作
[GC (Allocation Failure) [PSYoungGen: 2048K->488K(2560K)] 2048K->680K(9728K), 0.0022181 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[GC (Allocation Failure) [PSYoungGen: 2536K->488K(2560K)] 2728K->720K(9728K), 0.0010245 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[GC (Allocation Failure) [PSYoungGen: 2536K->504K(2560K)] 2768K->776K(9728K), 0.0012706 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
100000
Heap
 PSYoungGen      total 2560K, used 1579K [0x00000000ffd00000, 0x0000000100000000, 0x0000000100000000)
  eden space 2048K, 52% used [0x00000000ffd00000,0x00000000ffe0cea0,0x00000000fff00000)
...
Number of entries       :     13237 =    317688 bytes, avg  24.000
Number of literals      :     13237 =    566408 bytes, avg  42.790
Total footprint         :           =   1044184 bytes
...
Number of entries       :     20485 =    491640 bytes, avg  24.000
Number of literals      :     20485 =   1206440 bytes, avg  58.894
Total footprint         :           =   2178184 bytes
Average bucket size     :     0.341
Variance of bucket size :     0.356
Std. dev. of bucket size:     0.597
...
- 因为StringTable是由HashTable实现的，所以可以**适当增加HashTable桶的个数**，来减少字符串放入串池所需要的时间
  ...
  通过增加桶的个数，即适当调整-XX:StringTableSize=xxxx数值大小，可增加性能。

- 考虑是否需要将字符串对象入池

  如果不把字符串对象入池，当面临大量重复字符串时，每个字符串都会占据独立的空间，大大浪费内存，字符串入池可大大减少重复字符串消耗的空间。
...
public class Demo1_26 {
    ...
    /*
     * -XX:+DisableExplicitGC 显式的 加入这段虚拟机参数会使System.gc();无效
     */
    public static void main(String[] args) throws IOException {
...
![image-20230228143323549](黑马jvm/image-20230228143323549.png)
...
[B@7ea987ac
3
[GC (Allocation Failure) [PSYoungGen: 1615K->488K(6144K)] 13903K->12947K(19968K), 0.0016821 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[B@12a3a380
4
[GC (Allocation Failure) --[PSYoungGen: 4809K->4809K(6144K)] 17268K->17324K(19968K), 0.0009541 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[Full GC (Ergonomics) [PSYoungGen: 4809K->4518K(6144K)] [ParOldGen: 12515K->12488K(13824K)] 17324K->17007K(19968K), [Metaspace: 3075K->3075K(1056768K)], 0.0038819 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[GC (Allocation Failure) --[PSYoungGen: 4518K->4518K(6144K)] 17007K->17015K(19968K), 0.0004397 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[Full GC (Allocation Failure) [PSYoungGen: 4518K->0K(6144K)] [ParOldGen: 12496K->606K(8704K)] 17015K->606K(14848K), [Metaspace: 3075K->3075K(1056768K)], 0.0046698 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[B@29453f44
5
循环结束：5
null
...
Heap
 PSYoungGen      total 6144K, used 4321K [0x00000000ff980000, 0x0000000100000000, 0x0000000100000000)
  eden space 5632K, 76% used [0x00000000ff980000,0x00000000ffdb8640,0x00000000fff00000)
  from space 512K, 0% used [0x00000000fff00000,0x00000000fff00000,0x00000000fff80000)
  to   space 512K, 0% used [0x00000000fff80000,0x00000000fff80000,0x0000000100000000)
 ParOldGen       total 8704K, used 606K [0x00000000fec00000, 0x00000000ff480000, 0x00000000ff980000)
  object space 8704K, 6% used [0x00000000fec00000,0x00000000fec97af0,0x00000000ff480000)
 Metaspace       used 3119K, capacity 4500K, committed 4864K, reserved 1056768K
  class space    used 339K, capacity 388K, committed 512K, reserved 1048576K
```

可以看到当空间面临不足时，软引用指向的对象被回收了。
...
弱引用也是用来描述那些非必须对象，但是它的强度比软引用更弱一些，被弱引用关联的对象只 能生存到下一次垃圾收集发生为止。当垃圾收集器开始工作，无论当前内存是否足够，都会回收掉只 被弱引用关联的对象。在JDK 1.2版之后提供了**WeakReference**类来实现弱引用。
...
- 这里需要注意的是，并非每个对象都有终结器引用。这里参考了下chatGpt的回答

![image-20230228184704253](黑马jvm/image-20230228184704253.png)
...
- 虚引用和终结器引用**必须配合**引用队列
  ...
  虚引用和终结器引用在使用时会自动关联一个引用队列

## 2 垃圾回收算法

### 2.1 标记-清除

![](黑马jvm/20200608150813.png)
...
![](黑马jvm/20200608150827.png)

标记-整理 会将不被GC Root引用的对象回收，清楚其占用的内存空间。然后整理剩余的对象，可以有效避免因内存碎片而导致的问题，但是因为整体需要消耗一定的时间，所以效率较低

### 2.3 复制

![img](黑马jvm/20200608150842.png)

![img](黑马jvm/20200608150856.png)

![img](黑马jvm/20200608150907.png)

![img](黑马jvm/20200608150919.png)

将内存分为等大小的两个区域，FROM和TO（TO中为空）。先将被GC Root引用的对象从FROM放入TO中，再回收不被GC Root引用的对象。然后交换FROM和TO。这样也可以避免内存碎片的问题，但是会占用双倍的内存空间。

## 3 分代回收
这部分详细内容请看 `深入理解Java虚拟机第三版  3.8　实战：内存分配与回收策略`

![img](黑马jvm/20200608150931.png)

###  3.1 回收流程

新创建的对象都被放在了**新生代的伊甸园**中

![img](黑马jvm/20200608150939.png)

当伊甸园中的内存不足时，就会进行一次垃圾回收，这时的回收叫做 **Minor GC**

Minor GC 会将**伊甸园和幸存区FROM**存活的对象**先**复制到 **幸存区 TO**中， 并让其**寿命加1**，再**交换两个幸存区**

![img](黑马jvm/20200608150946.png)

![img](黑马jvm/20200608150955.png)

![img](黑马jvm/20200608151002.png)
...
![img](黑马jvm/20200608151010.png)

如果幸存区中的对象的**寿命超过某个阈值**（最大为15，4bit），就会被**放入老年代**中

![img](黑马jvm/20200608151018.png)

如果新生代老年代中的内存都满了，就会先触发Minor GC，再触发**Full GC**，扫描**新生代和老年代中**所有不再使用的对象并回收

**分代回收的过程（参考自chatgpt)**

当我们创建一个对象时，JVM 将会把这个对象分配到新生代的 Eden 区。如果 Eden 区空间不足，JVM 就会触发 Minor GC，对 Eden 区进行垃圾回收。

在 Minor GC 过程中，JVM 将会遍历 Eden 区和 Survivor 区中所有的对象，标记所有存活的对象，并将它们复制到 Survivor 区中的另一个空闲区域。复制后，清空 Eden 区和之前使用的 Survivor 区。在复制过程中，如果 Survivor 区的空间不足以存放存活的对象，这些对象就会被直接移动到老年代中。

为了防止 Survivor 区空间的不足，JVM 通常会设置两个 Survivor 区，分别称为 Survivor1 和 Survivor2。当一个 Survivor 区被填满时，JVM 会将其中的存活对象复制到另一个 Survivor 区中，并清空该 Survivor 区。同时，JVM 会调整存活对象的年龄。每个对象被复制到 Survivor 区时，它的年龄就会加 1。当一个对象的年龄达到一定阈值（通常是 15），它就会被移动到老年代中。

在老年代中，当内存空间不足以分配新对象时，JVM 将会触发 Major GC，对整个老年代进行垃圾回收。Major GC 的过程与 Minor GC 类似，但由于老年代中的对象存活时间更长，所以 Major GC 的触发频率相对较低。

分代回收的主要优势在于，由于新生代中的对象生命周期较短，所以它们的垃圾回收可以更加频繁、快速。而老年代中的对象生命周期较长，垃圾回收的频率相对较低，但垃圾回收的成本更高。通过将内存分为不同的代，JVM 可以根据不同代的特点，采用不同的垃圾回收算法，从而优化垃圾回收的效率。
...
![](黑马jvm/image-20230301174410796-1677663852193-25.png)
...
![img](黑马jvm/20200608151039-1677909118636-13.png)
...
![img](黑马jvm/20200909201212-1677909625100-99.png)
...
![img](黑马jvm/20200608151100-1677909629340-102.png)

##### G1垃圾回收阶段

![img](黑马jvm/20200608151109-1677909241060-18.png)
...
![img](黑马jvm/20200608151119-1677909245802-21.png)

![img](黑马jvm/20200608151129-1677909256623-24.png)

![img](黑马jvm/20200608151140-1677909260018-27.png)
...
![img](黑马jvm/20200608151150-1677909263619-30.png)
...
![img](黑马jvm/20200608151201-1677909267249-33.png)
...
![img](黑马jvm/20200608151211-1677909052519-4.png)
...
![img](黑马jvm/20200608151222-1677909274408-36.png)
...
![img](黑马jvm/20200608151229-1677909044776-1.png)
...
![img](黑马jvm/20200608151239-1677909060294-7.png)
...
[![img](黑马jvm/20200608151249-1677909064833-10.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200608151249.png)
...
[![img](黑马jvm/20200608151317.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200608151317.png)

**方法字节码载入方法区**

（stack=2，locals=4） 对应操作数栈有2个空间（每个空间4个字节），局部变量表中有4个槽位

[![img](黑马jvm/20200608151325.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200608151325.png)
...
![img](黑马jvm/20200608151346.png)

![img](黑马jvm/20200608151412.png)
...
![img](黑马jvm/20200608151421.png)

**istore 2**

将操作数栈中的元素弹出，放到局部变量表的2号位置

![img](黑马jvm/20200608151432.png)

![img](黑马jvm/20200608151441.png)
...
![img](黑马jvm/20200608151450.png)

![img](黑马jvm/20200608151459.png)

**iadd**

将操作数栈中的两个元素**弹出栈**并相加，结果在压入操作数栈中

![img](黑马jvm/20200608151508.png)

![img](黑马jvm/20200608151523.png)

**istore 3**

将操作数栈中的元素弹出，放入局部变量表的3号位置

![img](黑马jvm/20200608151547.png)

![img](黑马jvm/20200608151555.png)
...
![img](黑马jvm/20200608151605.png)

![img](黑马jvm/20200608151613.png)

**iload 3**

将局部变量表中3号位置的元素压入操作数栈中

![img](黑马jvm/20200608151624.png)
...
![img](黑马jvm/20200608151632.png)

执行完毕，弹出栈帧

清除 main 操作数栈内容

![img](黑马jvm/20200608151640.png)
...
[![img](黑马jvm/20200611205050.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200611205050.png)
...
[![img](黑马jvm/20200611221703.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200611221703.png)

- 定位需要的进程

[![img](黑马jvm/20200611221857.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200611221857.png)

[![img](黑马jvm/20200611222029.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200611222029.png)
...
[![img](黑马jvm/20200611223153.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200611223153.png)

查看类C的常量池，可以看到类D**未被解析**，只是存在于常量池中的符号

[![img](黑马jvm/20200611230658.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200611230658.png)

- 解析以后，会将常量池中的符号引用解析为直接引用

  - 可以看到，此时已加载并解析了类C和类D

  [![img](黑马jvm/20200611223441.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200611223441.png)

[![img](黑马jvm/20200613104723.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200613104723.png)
...
[![img](黑马jvm/20200614133554.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200614133554.png)
...
[![img](黑马jvm/20200614135011.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20200614135011.png)
